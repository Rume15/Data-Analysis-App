...
def analyze_top_fields(nil_importers, group_by_cols, agg_dict, sort_by, top_n, csv_filename, empty_fields, display_mode):
    ...
    # Automatically add fields aggregated as 'first' to empty_fields
    empty_fields += [key for key, value in agg_dict.items() if value == 'first']
    ...
    top_field_amounts = (nil_importers.groupby(group_by_cols)
                         .agg(agg_dict)
    ...
    if add_totals:
        ...
        totals_row = pd.DataFrame([{
            ...
            **{key: nil_importers[key].sum() if agg_dict[key] == 'sum' else nil_importers[key].nunique() if agg_dict[key] == 'nunique' else '' for key in agg_dict.keys()},
    ...
    analysis_result.to_csv(csv_filename, index=None)
